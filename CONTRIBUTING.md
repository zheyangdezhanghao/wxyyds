...
## Framework Hook 偏移（可选）

聊天内灰字等 Runtime Hook 偏移放在 `offsets/hook_<build>.json`，与 Patch offsets 分离。示例见 `offsets/hook_269077.json`。
...
- [ ] 更新了 `manifest.json`
- [ ] Hook 偏移（如有）更新了 `offsets/hook_<build>.json` 与 `Rely/supported_versions.txt`
